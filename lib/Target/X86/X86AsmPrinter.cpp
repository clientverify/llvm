...
unsigned int X86AsmPrinter::getPhysRegSize(unsigned int reg) const {
  if (X86MCRegisterClasses[X86::GR64RegClassID].contains(reg))
    return 8;
  if (X86MCRegisterClasses[X86::GR32RegClassID].contains(reg))
    return 4;
  if (X86MCRegisterClasses[X86::GR16RegClassID].contains(reg))
    return 2;
  if (X86MCRegisterClasses[X86::GR8RegClassID].contains(reg))
    return 1;
  llvm_unreachable("Unrecognized register - cannot determine its size");
}
